...
impl Config {
    ...
    pub(crate) fn add_copyright_asset(&mut self) -> CDResult<()> {
        let (source_path, copyright_file) = crate::data::generate_copyright_asset(self)?;
    ...
    fn add_changelog_asset(&mut self) -> CDResult<()> {
        ...
        if self.changelog.is_some() {
            if let Some((source_path, changelog_file)) = crate::data::generate_changelog_asset(self)? {
